...
plotsim <- function(sim){
    ...
    ggplot(aes(x=t, y=q), data=sim$q)+
        ...
        geom_segment(aes(x=sim$last.cust.arr,xend=sim$last.cust.arr, y=0,yend=max(sim$q$q)+1.8),size=2,color="red")+
        labs(x="Time (Hours)",y="Queue Length", title = "Queue Length at Time t")+
        annotate("text",x=sim$last.cust.arr,y=max(sim$q$q)+2,label="Last Arrival",color="red")+
        annotate("text", hjust=0, vjust=1, x=0, y=max(sim$q$q)+3,label=instr, color="blue")+
        annotate("text", hjust=1, vjust=1, x=max(sim$q$t), y=max(sim$q$q)+3,label=outstr, color="red")
}